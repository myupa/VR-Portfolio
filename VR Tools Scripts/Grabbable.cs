...
namespace BNG {
    ...
    /// <summary>
    /// Modified Version of BNG's Grabbable Script - Emmy Yupa
    /// </summary>

    public class Grabbable : MonoBehaviour {

        //Emmy Yupa Edits Start Here
        //

        [HideInInspector] public bool guideHandsInstantiated = false;
        List<GameObject> hands = new List<GameObject>();

        public void Start()
        {
            VRToolsManager.instance.showGuideHands += EnableGrabberGuideHands;
            VRToolsManager.instance.hideGuideHands += DisableGrabberGuideHands;

            if (OtherGrabbableMustBeGrabbed != null)
            {
                OtherGrabbableMustBeGrabbed.isSecondaryGrabbable = true;
            }
        }
        ...
        public void DisableGrabberGuideHands()
        {
            if (guideHandsInstantiated)
            {
                guideHandsInstantiated = false;
                if (hands != null)
                {
        ...
        [HideInInspector] public bool isSecondaryGrabbable;

        public void EnableGrabberGuideHands()
        {
            if (!guideHandsInstantiated)
            {
                guideHandsInstantiated = true;
                if (!isSecondaryGrabbable) { EnableMainHand(); } else { EnableSecondaryHand(); }
            }
        }

        public void EnableMainHand()
        {
            bool handReady = false; //Only instantiates one guidehand for one grab point per Grabbable. Stops as soon as one is ready.

            foreach (Transform point in GrabPoints)
            {
                if (!handReady && point != null && point.TryGetComponent(out BNG.GrabPoint grabScript))
                {
                    if (VRToolsManager.instance.dominantHand == VRToolsManager.HandDominance.LeftHanded)
                    {
                        if (grabScript.LeftHandIsValid) { handReady = true; InstantiateLeftHand(point); }
                    }
                    else if (VRToolsManager.instance.dominantHand == VRToolsManager.HandDominance.RightHanded)
                    {
                        if (grabScript.RightHandIsValid) { handReady = true; InstantiateRightHand(point); }
                    }
                    else if (VRToolsManager.instance.dominantHand == VRToolsManager.HandDominance.Ambidextrous)
                    {
                        if (grabScript.RightHandIsValid) { InstantiateRightHand(point); }
                        if (grabScript.LeftHandIsValid) { InstantiateLeftHand(point); }
                        handReady = true;
                    }
                }
            }
        ...
        public void EnableSecondaryHand()
        {
            bool handReady = false; //Only instantiates one guidehand for one grab point per Grabbable. Stops as soon as one is ready.
        ...
        void InstantiateLeftHand(Transform point)
        {
            GameObject hand = Instantiate(Resources.Load("GrabGuideL", typeof(GameObject))) as GameObject;
            hand.transform.parent = point.transform;
            hand.transform.localPosition = Vector3.zero;
            hand.transform.localEulerAngles = new Vector3(0f, 0f, 0f);//90f);
            hands.Add(hand);

        }

        void InstantiateRightHand(Transform point)
        {
            GameObject hand = Instantiate(Resources.Load("GrabGuideR", typeof(GameObject))) as GameObject;
            hand.transform.parent = point.transform;
            hand.transform.localPosition = Vector3.zero;
            hand.transform.localEulerAngles = new Vector3(0f, 0f, 0f);// -90f);
            hands.Add(hand);
        ...
        //
        //Emmy Yupa Edits End Here. Additional Edit at line 665.
        ...
        public virtual void Update() {
            ...
            if (BeingHeld) {
                ...
                //Last Edit By Emmy Yupa
                if (VRToolsManager.instance.showingGuideHands) { DisableGrabberGuideHands(); }
